...
class DeleteMan:
    def __init__(self, model_file, input_path, output_path, valid_path=None, size_x=64, size_y=64):
        ...
        self.x = np.empty(shape=(0,size_x,size_y,3))
        self.y = np.empty(shape=(0,size_x,size_y,3))
        self.valid = np.empty(shape=(0,size_x,size_y,3))
        ...
        try:
            ...
            inputs = tf.keras.Input(shape=(size_x,size_y,3))
            self.model = get_model(inputs)

        for filename in glob.glob(str(input_path)+"/*.jpg"):
            image = Image.open(filename).resize(size=(self.size_x,self.size_y))
            image_array = np.asarray(image).reshape(self.size_x,self.size_y,3)/self.divide_color
            self.x = np.append(self.x, [image_array], axis=0)

        for filename in glob.glob(str(output_path)+"/*.jpg"):
            image = Image.open(filename).resize(size=(self.size_x,self.size_y))
            image_array = np.asarray(image).reshape(self.size_x,self.size_y,3)/self.divide_color
        ...
        if valid_path == None:
            ...
            self.hasvalid = True
            for filename in glob.glob(str(valid_path)+"/*.jpg"):
    ...
    def generate_image(self, image):
        ...
        if len(np.shape(image)) == 4:
            predicts = self.model.predict(image).reshape(-1,self.size_x,self.size_y,3)*self.divide_color
            for i in predicts:
                result = Image.fromarray(i.astype(np.uint8)).resize(size=(288,288))
            ...
            predicts = self.model.predict(image.reshape(1,self.size_x,self.size_y,3)).reshape(-1,self.size_x,self.size_y)*self.divide_color
